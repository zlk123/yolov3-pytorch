...
class DetectionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        decode_pred = torch.cat((decode_l, decode_m, decode_h), 1).view(-1, l_grid_wh[0]*l_grid_wh[1]+ m_grid_wh[0]*m_grid_wh[1]
                                                 +h_grid_wh[0]*h_grid_wh[1] , 3, 5+ self.num_classes)                   
        
        
        decode_pred_maxconf,_ = torch.max(decode_pred[...,4:5], dim = 2, keepdim= True) 
